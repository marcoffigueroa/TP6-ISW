...
def test_confirmar_pago_devuelve_resumen_correcto():
    # Verificamos que la función exista
    try:
        from comprar_entradas.views import confirmar_pago
    except ImportError:
    ...
    orden_existente = {
        ...
        "estado": "PENDIENTE",
        "lineas": [
            {"visitante": {"nombre": "Ana"}, "precio": {"monto": 10000}},
            {"visitante": {"nombre": "Luis"}, "precio": {"monto": 15000}},
        ],
        "fecha_visita": "2025-10-20"
    }
    store = {orden_id: orden_existente}
    ...
    # Assert
    assert isinstance(resultado, dict), "El resultado debe ser un diccionario"
    assert "cantidad_entradas" in resultado, "Debe devolver la cantidad de entradas"
    assert "fecha_visita" in resultado, "Debe devolver la fecha de visita"
